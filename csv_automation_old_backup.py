...
def clean_column_name(name: str) -> str:
    """Clean column name for SQL compatibility with capitalization"""
    name = str(name).strip()
...
class CSVAutomationWorker(QThread):
    ...
    # cleanup_temp_files method removed - no longer needed as we process directly to database
    ...
    def load_dataframe_to_database(self, df, table_name, mode='replace'):
        """Load a DataFrame directly into the database table.
        
        Args:
            df: DataFrame to load
            table_name: Name of the target table
            mode: 'replace' to create/replace table, 'append' to append to existing table
        """
        try:
            ...
            logger.info(f"Successfully loaded {len(df)} rows into {table_name} (mode: {mode})")
    ...
    def process_large_csv_direct_to_db(self, file_path, table_name, file_name, chunk_size, unified_columns=None, mode='replace'):
        """Process large CSV files in chunks and load directly into database."""
        total_rows = 0
        first_chunk = True
        ...
        try:
            for chunk_num, chunk in enumerate(pd.read_csv(file_path, chunksize=chunk_size, encoding='utf-8')):
                ...
                # Clean column names and add source file
                ...
                if first_chunk:
                    # Create table with first chunk using the specified mode
                    self.load_dataframe_to_database(chunk, table_name, mode=mode)
                    first_chunk = False
                else:
                    # Append subsequent chunks
                    self.load_dataframe_to_database(chunk, table_name, mode='append')
                ...
                self.progress.emit(
                    ...
                    f"Loaded chunk {chunk_num + 1}: {total_rows:,} rows into {table_name}"
                )
                
                del chunk  # Free memory
                
        except Exception as e:
            logger.error(f"Error processing CSV chunks for {file_path}: {e}")
            raise
        
        return total_rows
    
    def process_large_excel_direct_to_db(self, file_path, table_name, file_name, chunk_size, sheet_selection, sheet_name, unified_columns=None, mode='replace'):
        """Process large Excel files in chunks and load directly into database."""
        total_rows = 0
        first_chunk = True
        
        try:
            # Read Excel file based on sheet selection
            if sheet_selection == 'First sheet only':
                ...
                if len(df) > chunk_size:
                    for i in range(0, len(df), chunk_size):
                        ...
                        self.progress.emit(
                            self.current_progress + 10,
                            f"Loaded chunk {i//chunk_size + 1}: {total_rows:,} rows into {table_name}"
                        )
                    ...
                    self.load_dataframe_to_database(df, table_name, mode=mode)
                    total_rows = len(df)
            ...
            logger.error(f"Error processing Excel chunks for {file_path}: {e}")
            raise
    ...
    def process_csv_source_with_progress(self, source_config, table_name, source_name):
        """Process CSV source (folder or single file) with progress reporting and direct database loading"""
        ...
        try:
            ...
            if mode in ['csv_file', 'excel_file']:
                ...
                if file_size_mb > LARGE_FILE_THRESHOLD_MB:
                    ...
                    if file_type == 'excel':
                        total_rows = self.process_large_excel_direct_to_db(file_path, table_name, file_name, CHUNK_SIZE, unified_columns=None)
                    else:
                        total_rows = self.process_large_csv_direct_to_db(file_path, table_name, file_name, CHUNK_SIZE, unified_columns=None)
                    ...
                    # Normal processing for smaller files - load directly to database
                    self.progress.emit(
                        self.current_progress + 10,
                        f"Reading {file_name}..."
                    )
                    ...
                    try:
                        ...
                        self.progress.emit(
                            ...
                            f"Loading {len(df):,} rows from {source_name} into database..."
                        )
                        
                        # Load directly into database instead of saving to CSV
                        self.load_dataframe_to_database(df, table_name)
                        ...
                        if file_type == 'csv':
                            ...
                            try:
                                ...
                                self.progress.emit(
                                    self.current_progress + 20,
                                    f"Loading {len(df):,} rows from {source_name} into database (as text)..."
                                )
                            ...
                            sheet_selection = source_config.get('sheet_selection', 'All sheets (combined)')
                            sheet_name = source_config.get('sheet_name', '')
                            total_rows = self.process_large_excel_direct_to_db(file_path, table_name, file_name, CHUNK_SIZE, sheet_selection, sheet_name, unified_columns=None)
                            return pd.DataFrame({'_row_count': [total_rows], '_source_file': [file_name]})
                ...
                for i, file_path in enumerate(files):
                    ...
                    try:
                        if use_chunked or file_size_mb > LARGE_FILE_THRESHOLD_MB:
                            # Process large files directly to database (no temporary files)
                            if file_size_mb > LARGE_FILE_THRESHOLD_MB:
                                if file_type == 'excel':
                                    sheet_selection = source_config.get('sheet_selection', 'All sheets (combined)')
                                    sheet_name = source_config.get('sheet_name', '')
                                    mode = 'replace' if first_file else 'append'
                                    rows_processed = self.process_large_excel_direct_to_db(file_path, table_name, file_name, CHUNK_SIZE, sheet_selection, sheet_name, unified_columns, mode=mode)
                                    first_file = False
                                else:
                                    mode = 'replace' if first_file else 'append'
                                    rows_processed = self.process_large_csv_direct_to_db(file_path, table_name, file_name, CHUNK_SIZE, unified_columns, mode=mode)
                                    first_file = False
                            else:
                                # Normal read and load directly to database
                                if file_type == 'excel':
                                    sheet_selection = source_config.get('sheet_selection', 'All sheets (combined)')
                                    sheet_name = source_config.get('sheet_name', '')
                                    df = self.read_excel_file(file_path, file_name, sheet_selection, sheet_name)
                                ...
                                if first_file:
                                    self.load_dataframe_to_database(df, table_name, mode='replace')
                                    first_file = False
                                else:
                                    self.load_dataframe_to_database(df, table_name, mode='append')
                            ...
                            # Normal processing for small files - load directly to database
                            try:
                                if file_type == 'excel':
                                    sheet_selection = source_config.get('sheet_selection', 'All sheets (combined)')
                                    sheet_name = source_config.get('sheet_name', '')
                                ...
                                # Load directly into database
                                if first_file:
                                    # First file - create/replace table
                                    self.load_dataframe_to_database(df, table_name, mode='replace')
                                    first_file = False
                                else:
    ...
    def run(self):
        try:
            ...
            for i, source_config in enumerate(self.sources_config):
                ...
                # Process data directly into database (no temporary files needed)
                
                try:
                    # Use our progress-enabled CSV processor with direct database loading
                    ...
                    # Data is already loaded into database by process_csv_source_with_progress
                    table_name = source_config['table_name']
                    
                    # Validate connection and check final table status
                    if not self.reconnect_if_needed():
                        raise ConnectionError("Failed to establish database connection")
                    ...
                    try:
                        ...
                        self.progress.emit(
                            ...
                            f"Completed loading {table_name}: {row_count:,} rows"
                        )
                        
                    except Exception as e:
                        logger.error(f"Error verifying table {table_name}: {e}")
                        raise ValueError(f"Failed to verify table creation for {table_name}: {e}")
                ...
                if file_type == 'excel':
                    ...
                    if sheet_selection == "First sheet only":
                        ...
                        if df_sample is not None and not df_sample.empty:
                            current_file_columns = [clean_column_name(col) for col in df_sample.columns.tolist()]
                    else:
                        ...
                        try:
                            ...
                            for sheet_name_iter in wb.sheetnames:
                                try:
                                    ...
                                    if df_sample is not None and not df_sample.empty:
                                        cleaned_columns = [clean_column_name(col) for col in df_sample.columns.tolist()]
                                        sheet_columns.update(cleaned_columns)
                                except Exception as e:
...
class CSVAutomationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Tab 3: Output (removed - no longer needed)
        # Output table names are now automatically generated
    ...
    def execute_automation(self):
        ...
        # No output table configuration needed
        output_config = {}
    ...
    def set_automation_config(self, config):
        ...
        try:
            ...
            # Output table configuration removed
